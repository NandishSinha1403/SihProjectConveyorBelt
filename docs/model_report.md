...
Weights: `belt_v1.pt`  
Dataset: `training/data/merged/data.yaml`  
Device: `mps`  
Generated: 2026-08-31 13:45

> Generated by `python training/evaluate.py`. Hand edits are overwritten on the
> next run — change the generator, not this file.
>
> This covers **`belt_v1`** only, the generalist trained on the public Roboflow
> data. `belt_v2`, the model specialised to the prototype rig, is scored by
> field test rather than validation mAP, because it and its validation split
> come from the same few minutes of footage and the metric would flatter it.
> Those figures are in the [README's Results section](../README.md#results).
